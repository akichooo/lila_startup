...
export default function AnalysisResultPreview({ result }: Props) {
  ...
  return (
    <div className="flex flex-col items-center gap-6 py-4">
      <Blobby size={180} state="speaking" />
      ...
      >
        ...
        <div className="grid grid-cols-2 gap-3 text-sm">
          <div>
            ...
            <p className="font-bold" style={{ color: "#2D1B69" }}>{result.groupName}</p>
          </div>
          <div>
            <p className="text-xs font-semibold" style={{ color: "#7C6FAA" }}>Duration</p>
            <p className="font-bold" style={{ color: "#2D1B69" }}>{result.duration} min</p>
          </div>
          <div>
            <p className="text-xs font-semibold" style={{ color: "#7C6FAA" }}>Date</p>
            <p className="font-bold" style={{ color: "#2D1B69" }}>{result.date}</p>
          </div>
          <div>
            <p className="text-xs font-semibold" style={{ color: "#7C6FAA" }}>Tone</p>
            <p className="font-bold" style={{ color: "#2D1B69" }}>{result.overallTone}</p>
          </div>
        </div>
          ...
          <div className="space-y-1.5">
            {result.studentResults.map((sr) => (
              <div key={sr.student.id} className="flex items-center gap-2 text-xs">
                <span className="w-20 truncate font-semibold" style={{ color: "#2D1B69" }}>
                  {sr.student.name.split(" ")[0]}
                </span>
                <div className="flex-1 h-2.5 rounded-full overflow-hidden" style={{ background: "#EDE9FF" }}>
                  <div
                    className="h-full rounded-full"
                    style={{
                      width: `${sr.participationPct}%`,
                      background: sr.flagged
                        ? "linear-gradient(90deg, #FDBA74, #FB7185)"
      ...
      <div className="flex gap-3 w-full max-w-lg">
        ...
        <button
          className="lila-btn-primary flex-1"
