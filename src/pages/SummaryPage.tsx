...
// Static data for the default summary (ses1)
const defaultStudentCards = [
...
function DynamicSummary({ result }: { result: AnalysisResult }) {
  const [rating, setRating] = useState(0);
  const engStyle = (eng: string) =>
    eng === "High" || eng === "Engaged" ? "lila-badge-green" : "lila-badge-amber";
    ...
    <AppShell pageTitle="Session Summary">
      <div className="lila-card p-4 mb-6 flex flex-wrap items-center justify-between gap-4">
        <div className="flex flex-wrap items-center gap-3 text-sm">
          <span className="font-bold" style={{ color: "#2D1B69" }}>{result.topic}</span>
          <span style={{ color: "#EDE9FF" }}>|</span>
          <span style={{ color: "#7C6FAA" }}>{result.groupName}</span>
          <span style={{ color: "#EDE9FF" }}>|</span>
          <span style={{ color: "#7C6FAA" }}>{result.date}</span>
          <span style={{ color: "#EDE9FF" }}>|</span>
          <span style={{ color: "#7C6FAA" }}>{result.duration} minutes</span>
          <span style={{ color: "#EDE9FF" }}>|</span>
          <span style={{ color: "#7C6FAA" }}>{result.studentCount} students</span>
      ...
      <div className="space-y-6 max-w-[900px] mx-auto">
        <div className="lila-card">
          ...
          <p className="text-sm leading-relaxed" style={{ color: "#2D1B69" }}>{result.summaryParagraph}</p>
        ...
        <div className="lila-card" style={{ borderColor: "#FDBA74", background: "linear-gradient(135deg, #FFFFFF 0%, #FFF7ED 100%)" }}>
          ...
          <div className="flex items-start gap-4 text-sm">
            <span className="text-xs shrink-0" style={{ color: "#A89DC4" }}>{result.misinfoCorrection.time}</span>
            <div>
              <p style={{ color: "#2D1B69" }}><span className="font-bold">{result.misinfoCorrection.studentName}</span> · {result.misinfoCorrection.topic}</p>
              <p className="mt-1" style={{ color: "#7C6FAA" }}>Original: "{result.misinfoCorrection.original}"</p>
              <p style={{ color: "#7C6FAA" }}>Reframed — {result.misinfoCorrection.reframe}</p>
            </div>
          </div>
        </div>
        ...
        <div className="lila-card">
          ...
          <div className="space-y-3">
            {result.highlights.map((h, i) => (
              <div key={i} className="flex items-start gap-3 text-sm rounded-2xl p-3" style={{ background: h.isWarning ? "#FFF7ED" : "#FFFBEB" }}>
                {h.isWarning ? (
                  <AlertTriangle className="h-4 w-4 shrink-0 mt-0.5" style={{ color: "#D97706" }} />
                ) : (
                  <Bookmark className="h-4 w-4 shrink-0 mt-0.5" style={{ color: "#A78BFA" }} />
                )}
                <div>
        ...
        <div>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            {result.studentResults.map((c, ci) => (
              <div key={c.student.id} className="lila-card p-5 relative overflow-hidden">
                ...
                <div className="flex items-center gap-3 mb-3 pt-1">
                  ...
                  <div>
                    ...
                    <p className="text-xs" style={{ color: "#7C6FAA" }}>{result.groupName}</p>
                  </div>
                </div>
                <div className="space-y-2 text-sm">
                  <div className="flex items-center justify-between">
                    <span style={{ color: "#7C6FAA" }}>Speaking turns</span>
                    <span className="font-bold" style={{ color: "#2D1B69" }}>{c.speakingTurns} ({c.participationPct}%)</span>
                  </div>
                  <div className="h-2 rounded-full overflow-hidden" style={{ background: "#EDE9FF" }}>
                    <div className="h-full rounded-full" style={{ width: `${c.participationPct}%`, background: "linear-gradient(90deg, #A78BFA, #FB7185)" }} />
                  </div>
                  <div className="flex items-center justify-between">
                    <span style={{ color: "#7C6FAA" }}>Engagement</span>
                    <span className={engStyle(c.engagement)}>{c.engagement}</span>
                  </div>
...
export default function SummaryPage() {
  ...
  // If an ID is provided, look for a dynamic analysis result
  if (id) {
    const result = analyses.find((a) => a.id === id);
    if (result) return <DynamicSummary result={result} />;
  }

  // Default static summary (original page)
  return (
    <AppShell pageTitle="Session Summary">
      <div className="lila-card p-4 mb-6 flex flex-wrap items-center justify-between gap-4">
        <div className="flex flex-wrap items-center gap-3 text-sm">
